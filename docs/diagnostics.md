...
The module is **vendored**, not shared: ``src/resolve_configurator/diag.py``. Every repo in the fleet
...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/resolve-configurator/` |
| Linux | `$XDG_STATE_HOME/resolve-configurator/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\resolve-configurator\logs\` |

`RESOLVE_CONFIGURATOR_LOG_DIR` overrides it. The path is printed on the first line of every
...
`resolve-configurator.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `RESOLVE_CONFIGURATOR_LOG`, default `INFO`.
...
```bash
resolve-configure --collect-diagnostics
```

Writes `resolve-configurator-diagnostics-<timestamp>.json` and prints its path. One file, so
